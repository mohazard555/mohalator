...
import { ArrowRight, Plus, Save, X, Search, Scale } from 'lucide-react';
...
const OpeningEntriesView: React.FC<OpeningEntriesViewProps> = ({ onBack }) => {
  ...
  const filteredAccounts = chartAccounts.filter(a => 
    a.type === 'ACCOUNT' && (a.name.includes(accountSearch) || a.code.includes(accountSearch))
  );
           ...
           <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-[3rem] border border-zinc-200 shadow-2xl overflow-hidden animate-in zoom-in-95">
              <div className="p-8 space-y-6 text-right" dir="rtl">
                 <div className="flex items-center justify-between border-b pb-5">
                    ...
                    <button onClick={() => setIsAdding(false)} className="p-2 hover:bg-zinc-100 rounded-xl transition-all"><X className="w-6 h-6 text-zinc-400" /></button>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-2 relative">
                       ...
                       <div className="relative">
                          <input 
                            type="text" 
                            className="w-full bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-100 p-4 rounded-2xl font-black outline-none focus:border-primary transition-all text-sm"
                          ...
                          {showResults && accountSearch && (
                            <div className="absolute top-full right-0 left-0 mt-2 bg-white dark:bg-zinc-900 border rounded-2xl shadow-2xl z-[300] max-h-60 overflow-y-auto animate-in fade-in">
                               {filteredAccounts.length === 0 ? (
                                 <div className="p-4 text-center text-xs text-zinc-400 italic">لا توجد نتائج</div>
                               ) : (
                                 filteredAccounts.map(acc => (
                                   <div key={acc.id} onClick={() => { setForm({...form, accountName: acc.name}); setAccountSearch(acc.name); setShowResults(false); }} className="p-3 border-b hover:bg-zinc-50 cursor-pointer flex justify-between items-center transition-colors">
                                      <div className="flex flex-col text-right"><span className="font-black text-sm">{acc.name}</span><span className="text-[8px] text-zinc-400 font-bold">{acc.reportType}</span></div>
                    ...
                    <div className="flex flex-col gap-2">
                       ...
                       <select className="bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-100 p-4 rounded-2xl font-black outline-none appearance-none cursor-pointer text-sm" value={form.accountType} onChange={e => setForm({...form, accountType: e.target.value as any})}>
                    ...
                    <div className="md:col-span-2 flex flex-col gap-2">
                       <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mr-2">ملاحظات السند</label>
                       <textarea 
                         value={form.notes} 
                         onChange={e => setForm({...form, notes: e.target.value})} 
                         placeholder="اكتب ملاحظات إضافية هنا..." 
                         className="bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-100 p-4 rounded-2xl font-bold outline-none h-24 resize-none focus:border-primary transition-all text-sm"
                       />
                    </div>
                 </div>
              </div>
              <div className="bg-zinc-50 dark:bg-zinc-800/50 p-6 flex justify-end gap-3 border-t">
