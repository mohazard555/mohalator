...
const JournalEntryView: React.FC<JournalEntryViewProps> = ({ onBack }) => {
  ...
  const handleSave = () => {
    ...
    // الترحيل التلقائي: نقوم بتحويل كل سطر في القيد إلى سجل في اليومية العامة.
    // هذه العملية تضمن تحديث رصيد الحساب في "دليل الحسابات" لأن الدليل يحسب الأرصدة من اليومية.
    const newJournalEntries: CashEntry[] = validRows.map(r => ({
        id: crypto.randomUUID(),
        date,
        statement: r.notes || mainDescription || `قيد رقم ${voucherNumber}`,
        receivedSYP: r.debit, // المدين يعامل كزيادة في رصيد الحساب باليومية
        paidSYP: r.credit,   // الدائن يعامل كنقص في رصيد الحساب باليومية
        receivedUSD: 0,
        paidUSD: 0,
        notes: mainDescription,
        type: 'قيد',
        voucherNumber,
        partyName: r.accountName // اسم الحساب هو مفتاح الربط مع الدليل
      }));

    localStorage.setItem('sheno_cash_journal', JSON.stringify([...newJournalEntries, ...jou]));
    alert('تم ترحيل سند القيد بنجاح. ستنعكس الأرصدة فوراً في دليل الحسابات والتقارير الختامية.');
    onBack();
  };

  // تجميع كافة الحسابات من كل المصادر (الدليل، الجهات، البنود)
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between no-print">
        ...
        <div className="flex gap-2">
           <button onClick={() => window.print()} className="bg-zinc-100 dark:bg-zinc-800 text-readable border border-zinc-200 dark:border-zinc-700 px-6 py-2.5 rounded-2xl font-black flex items-center gap-2">
              <FileText className="w-5 h-5" /> معاينة الطباعة
           </button>
           <button onClick={handleSave} className="bg-primary text-white px-10 py-2.5 rounded-2xl font-black flex items-center gap-2 shadow-xl shadow-primary/20 hover:scale-105 transition-all">
              <Save className="w-5 h-5" /> ترحيل القيد للدليل
           </button>
        </div>
      </div>
                  ...
                  {rows.map((row, index) => (
                     <tr key={row.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors h-16 group">
                        ...
                        <td className="p-2 border-l dark:border-zinc-800 relative">
                           <div 
                              className={`w-full p-2.5 rounded-xl border-2 transition-all cursor-text flex items-center justify-between ${activeRowId === row.id ? 'border-primary bg-primary/5' : 'border-transparent hover:border-zinc-200 dark:hover:border-zinc-700'}`}
                              onClick={() => { 
                                 setActiveRowId(row.id); 
                                 setShowResults(true); 
                                 setAccountSearch(row.accountName); 
                              }}
                           >
                              <span className={row.accountName ? 'text-readable' : 'text-zinc-300 italic text-xs'}>
                                 {row.accountName || 'ابحث واضغط لاختيار الحساب...'}
                              </span>
                             ...
                             <div className="absolute top-full right-2 left-2 mt-1 bg-white dark:bg-zinc-950 border-2 border-primary rounded-2xl shadow-2xl z-[100] max-h-60 overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                                <div className="p-2 bg-zinc-50 dark:bg-zinc-900 border-b dark:border-zinc-800">
                                   <div className="relative">
                                      ...
                                      <input 
                                         type="text" 
                                         autoFocus 
                                         className="w-full bg-white dark:bg-zinc-800 border dark:border-zinc-700 rounded-xl py-2 pr-10 pl-4 outline-none font-black text-xs text-readable" 
                                         placeholder="اكتب اسم الحساب أو الكود..."
                                         value={accountSearch}
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar">
                                   {filteredResults.length === 0 ? (
                                      <div className="p-6 text-center text-zinc-400 text-xs italic">لا يوجد حساب مطابق</div>
                                   ) : (
                                      filteredResults.map((acc, i) => (
                                         <div 
                                            key={i} 
                                            onClick={() => {
                                               updateRow(row.id, 'accountName', acc.name);
                                               setShowResults(false);
                                               setActiveRowId(null);
                                            }}
                                            className="p-3 border-b dark:border-zinc-800 hover:bg-primary/10 cursor-pointer flex justify-between items-center group transition-colors"
                                         >
                                            <div className="flex flex-col text-right">
                                               <span className="font-black text-xs text-readable group-hover:text-primary">{acc.name}</span>
                                               <span className="text-[9px] text-zinc-400 font-bold uppercase">{acc.type}</span>
                                            </div>
                        ...
                        <td className="p-2 border-l dark:border-zinc-800 bg-emerald-50/10 dark:bg-emerald-950/10">
                           <input 
                              type="number" 
                              className="w-full bg-transparent border-b border-transparent focus:border-emerald-500 text-center font-mono font-black text-lg text-emerald-600 outline-none p-2"
                              value={row.debit || ''}
                              onChange={e => updateRow(row.id, 'debit', Number(e.target.value))}
                              onFocus={() => { if(row.credit > 0) updateRow(row.id, 'credit', 0); }}
                           />
                        </td>

                        <td className="p-2 border-l dark:border-zinc-800 bg-rose-50/10 dark:bg-rose-950/10">
                           <input 
                              type="number" 
                              className="w-full bg-transparent border-b border-transparent focus:border-rose-500 text-center font-mono font-black text-lg text-rose-600 outline-none p-2"
                              value={row.credit || ''}
                              onChange={e => updateRow(row.id, 'credit', Number(e.target.value))}
                              onFocus={() => { if(row.debit > 0) updateRow(row.id, 'debit', 0); }}
                           />
                        </td>

                        <td className="p-2 border-l dark:border-zinc-800">
                           <input 
                              type="text" 
                              placeholder="بيان السطر..." 
                              className="w-full bg-transparent border-b border-transparent focus:border-zinc-400 text-readable outline-none p-2 font-normal italic text-xs"
                              value={row.notes}
         ...
         <div className="p-8 bg-zinc-900 text-white flex flex-col md:flex-row items-center justify-between gap-8 border-t-4 border-primary">
            <div className="flex gap-10">
               <div className="flex flex-col items-center">
                  <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">إجمالي المدين (+)</span>
                  <span className="text-3xl font-mono font-black text-emerald-400">{totalDebit.toLocaleString()}</span>
               </div>
               <div className="flex flex-col items-center">
                  <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">إجمالي الدائن (-)</span>
                  <span className="text-3xl font-mono font-black text-rose-400">{totalCredit.toLocaleString()}</span>
               </div>
               <div className="w-px h-12 bg-zinc-800 hidden md:block"></div>
               <div className="flex flex-col items-center">
                  <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1">الفرق (التوازن)</span>
                  <div className="flex items-center gap-3">
                     <span className={`text-4xl font-mono font-black ${diff === 0 ? 'text-blue-500' : 'text-rose-600 animate-pulse'}`}>
            ...
            <div className="flex gap-3">
               <button onClick={handleAddRow} className="bg-zinc-800 hover:bg-zinc-700 text-white px-8 py-4 rounded-2xl font-black flex items-center gap-3 transition-all border border-zinc-700">
                  <Plus className="w-6 h-6" /> إضافة سطر جديد
               </button>
               <button 
                 onClick={handleSave} 
                 className={`px-16 py-4 rounded-2xl font-black text-xl shadow-2xl transition-all flex items-center gap-3 ${diff === 0 ? 'bg-primary hover:scale-105 active:scale-95' : 'bg-zinc-800 text-zinc-600 cursor-not-allowed opacity-50'}`}
                 disabled={diff !== 0}
               >
                  <Save className="w-7 h-7" /> ترحيل القيد ومزامنة الدليل
               </button>
            </div>
         </div>
      </div>
      
      <div className="p-6 bg-primary/5 border-2 border-primary/20 rounded-[2rem] flex items-center gap-4 text-readable no-print">
         <Calculator className="w-10 h-10 text-primary shrink-0" />
         <p className="text-sm font-bold leading-relaxed">
            <b>تنبيه محاسبي:</b> ترحيل القيد سيقوم بتحديث رصيد الحساب المختار فوراً في دليل الحسابات. يتم الربط بناءً على "اسم الحساب" لضمان التوافق مع الحسابات الحالية والمستقبلية.
         </p>
      </div>
    </div>
  );
