...
import { ArrowRight, Coins, CreditCard, Search, FileSpreadsheet, ImageIcon, Printer, Calendar, TrendingUp, TrendingDown, History, X } from 'lucide-react';
...
const DollarBalancesView: React.FC<DollarBalancesViewProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    if (savedCash) setEntries(JSON.parse(savedCash));
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 no-print">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
             <div className="p-3 bg-amber-500/10 text-amber-600 rounded-2xl"><CreditCard className="w-8 h-8" /></div>
             <div>
                <h2 className="text-2xl font-black text-readable">مركز أرصدة وحركات الدولار ($)</h2>
                <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest">تتبع شامل لكافة العملات الصعبة والمدفوعات الأجنبية</p>
             </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
         <div className="lg:col-span-1 space-y-6 no-print">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-xl space-y-4">
               <h3 className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.3em] border-b pb-2 mb-4">قائمة الحسابات بالدولار</h3>
               <div className="relative mb-4">
                  <Search className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                  <input type="text" placeholder="بحث باسم الحساب..." className="w-full bg-zinc-50 dark:bg-zinc-950 p-3 pr-10 rounded-2xl font-bold outline-none border dark:border-zinc-800 focus:border-amber-500 transition-all" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
               </div>
               <div className="space-y-2 max-h-[500px] overflow-y-auto custom-scrollbar pr-2">
                  {dollarBalances.length === 0 ? (
                    <p className="text-center py-10 text-xs text-zinc-400 font-bold italic">لا يوجد حركات بالدولار حالياً</p>
                  ) : dollarBalances.map((b, i) => (
                    <button key={i} onClick={() => setSelectedParty(b.name)} className={`w-full text-right p-4 rounded-2xl border transition-all flex flex-col gap-1 ${selectedParty === b.name ? 'bg-amber-500/10 border-amber-500/30 ring-2 ring-amber-500/20' : 'bg-zinc-50 dark:bg-zinc-800 border-transparent hover:bg-zinc-100'}`}>
         ...
         <div ref={reportRef} className="lg:col-span-3 space-y-6 export-fix bg-white dark:bg-zinc-950 p-6 md:p-10 rounded-[3rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 no-print-visible">
               ...
               <div className="bg-amber-500/10 p-8 rounded-[2rem] border border-amber-500/20 flex flex-col items-center gap-2">
            ...
            {selectedParty ? (
              <div className="space-y-6 animate-in slide-in-from-left-4">
                 <div className="flex items-center justify-between border-b-4 border-amber-500 pb-4">
                    <div className="flex items-center gap-4">
                       <div className="w-14 h-14 bg-zinc-900 rounded-2xl flex items-center justify-center text-white"><History className="w-8 h-8" /></div>
                       <div>
                          <h3 className="text-2xl font-black text-readable">حركات الحساب: {selectedParty}</h3>
                          <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest">تفصيل المقبوضات والمدفوعات بالدولار</p>
                       </div>
                    </div>
                 ...
                 <div className="overflow-x-auto rounded-[2rem] border border-zinc-100 dark:border-zinc-800 overflow-hidden shadow-sm">
                    <table className="w-full text-right border-collapse text-sm">
                       ...
                       <tbody className="divide-y font-bold dark:divide-zinc-800">
                          {selectedPartyMovements.length === 0 ? (
                            <tr><td colSpan={5} className="p-20 text-center italic text-zinc-400 font-bold">لا يوجد حركات لهذا الحساب</td></tr>
              ...
              <div className="h-[400px] flex flex-col items-center justify-center border-2 border-dashed border-zinc-100 dark:border-zinc-800 rounded-[3rem] text-center p-10 animate-pulse">
                 <Coins className="w-20 h-20 text-zinc-100 dark:text-zinc-800 mb-6" />
                 <h3 className="text-2xl font-black text-zinc-300">يرجى اختيار حساب من القائمة الجانبية</h3>
                 <p className="text-zinc-400 max-w-sm mt-2 font-bold">بمجرد اختيار حساب، ستظهر هنا كافة الحركات المالية التي تمت بالدولار الأمريكي بشكل تفصيلي ومبسط.</p>
              </div>
            )}
         </div>
      </div>
