...
import { 
  ...
  Users, ShoppingBag, ArrowLeftRight, Activity, BarChart3, Eye, EyeOff, Search
} from 'lucide-react';
import { 
...
const AccountingCenterView: React.FC<AccountingCenterViewProps> = ({ onBack }) => {
  ...
  const calculateFinancials = () => {
    ...
    const totalRevenues = revenueCats.reduce((s, c) => s + c.total, 0) + directSalesTotal;
    ...
    const purchasesTotal = journal.filter(j => j.type === 'شراء').reduce((s, c) => s + c.paidSYP, 0);
    ...
    const openingLiabilitiesItems = openingEntries.filter(e => e.accountType === 'خصوم');
    const liabOpening = openingLiabilitiesItems.reduce((s, c) => s + (c.credit - c.debit), 0);
    
    const openingEquityItems = openingEntries.filter(e => e.accountType === 'حقوق ملكية');
    const equityOpening = openingEquityItems.reduce((s, c) => s + (c.credit - c.debit), 0);

    return { 
      totalRevenues, totalExpenses, cogs, grossProfit, netProfit, 
      ...
      openingLiabilitiesItems, liabOpening, 
      openingEquityItems, equityOpening,
      revenueCats, expenseCats, directSalesTotal, purchasesTotal
    };
  };
  ...
  // Add missing handleDeleteEntry function
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 no-print">
        ...
        <div className="flex gap-2">
           {activeTab === 'REPORTS' && reportType === 'BALANCE_SHEET' && (
             <button onClick={() => setPrintDetails(!printDetails)} className={`px-4 py-2.5 rounded-2xl font-black text-xs flex items-center gap-2 border transition-all ${printDetails ? 'bg-primary text-white border-primary shadow-lg' : 'bg-zinc-100 text-zinc-500 border-zinc-200'}`}>
                {printDetails ? <Eye className="w-4 h-4"/> : <EyeOff className="w-4 h-4"/>}
                {printDetails ? 'إظهار التفاصيل في الطباعة' : 'ملخص فقط في الطباعة'}
             </button>
           )}
      ...
      <div className="flex bg-zinc-100 dark:bg-zinc-900 p-2 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 no-print">
         <button onClick={() => setActiveTab('REPORTS')} className={`flex-1 py-4 rounded-3xl font-black text-sm flex items-center justify-center gap-3 transition-all ${activeTab === 'REPORTS' ? 'bg-primary text-white shadow-xl' : 'text-zinc-500 hover:text-zinc-700'}`}>
            <PieChart className="w-5 h-5" /> الحسابات الختامية والتحليل
         </button>
         <button onClick={() => setActiveTab('OPENING_ENTRY')} className={`flex-1 py-4 rounded-3xl font-black text-sm flex items-center justify-center gap-3 transition-all ${activeTab === 'OPENING_ENTRY' ? 'bg-primary text-white shadow-xl' : 'text-zinc-500 hover:text-zinc-700'}`}>
            <Scale className="w-5 h-5" /> القيد الافتتاحي العام
         </button>
         <button onClick={() => setActiveTab('INVENTORY_TOOLS')} className={`flex-1 py-4 rounded-3xl font-black text-sm flex items-center justify-center gap-3 transition-all ${activeTab === 'INVENTORY_TOOLS' ? 'bg-primary text-white shadow-xl' : 'text-zinc-500 hover:text-zinc-700'}`}>
            <Box className="w-5 h-5" /> إدارة مخزون (أول / آخر) المدة
         </button>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6 no-print">
           ...
           {activeTab === 'OPENING_ENTRY' && (
             <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-lg space-y-4">
                ...
                <p className="text-xs text-zinc-500 font-bold leading-relaxed">لتأسيس أرصدة الأصول والخصوم والحسابات غير المخزنية.</p>
           ...
           {activeTab === 'INVENTORY_TOOLS' && (
             <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-lg space-y-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                   <button onClick={() => { setInvType('OPENING'); setIsAddingInventory(true); setInvItems([]); }} className="bg-emerald-600 text-white p-4 rounded-2xl font-black text-sm shadow-md hover:brightness-110 flex items-center justify-center gap-2">
                      <TrendingUp className="w-5 h-5" /> تسجيل جرد أول المدة
                   </button>
                   <button onClick={() => { setInvType('CLOSING'); setIsAddingInventory(true); setInvItems([]); }} className="bg-rose-900 text-white p-4 rounded-2xl font-black text-sm shadow-md hover:brightness-110 flex items-center justify-center gap-2">
                      <TrendingDown className="w-5 h-5" /> تسجيل جرد آخر المدة
                   </button>
                </div>
             </div>
           )}
        ...
        <div className="lg:col-span-3">
           {activeTab === 'REPORTS' && (
             <div ref={reportRef} className="bg-white dark:bg-zinc-950 p-6 md:p-10 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl export-fix min-h-[800px]">
                   ...
                   <div className="text-center">
                      <h2 className="text-3xl font-black underline decoration-primary/20 underline-offset-8">
                         {reportType === 'BALANCE_SHEET' ? 'الميزانية العمومية للفترة' : 
                          reportType === 'INCOME_STATEMENT' ? 'قائمة الأرباح والخسائر' : 'حساب المتاجرة وتحليل المخزون'}
                      </h2>
                ...
                {reportType === 'BALANCE_SHEET' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-zinc-900 dark:text-zinc-100">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="divide-y border border-zinc-100 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-inner">
                           <div className={`p-4 bg-zinc-50 dark:bg-zinc-900 font-bold text-sm flex flex-col gap-2 ${expandedBS.has('eq') ? 'bg-zinc-100 dark:bg-zinc-800' : ''}`}>
                             <div className="flex justify-between items-center cursor-pointer group" onClick={() => toggleBS('eq')}>
                                <span className="flex items-center gap-2 group-hover:text-primary">{expandedBS.has('eq') ? <MinusSquare className="w-4 h-4 text-zinc-400"/> : <PlusSquare className="w-4 h-4 text-primary"/>} رأس المال وحقوق الملكية</span>
                                <span className="font-mono">{fin.equityOpening.toLocaleString()}</span>
                             </div>
                           </div>
                           <div className="flex justify-between p-4 bg-white dark:bg-zinc-800 font-bold text-sm"><span>صافي الربح التقديري للفترة</span><span className="font-mono text-emerald-600">+{fin.netProfit.toLocaleString()}</span></div>
                        </div>
                ...
                {reportType === 'INCOME_STATEMENT' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 text-zinc-900 dark:text-zinc-100">
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6 no-print-visible">
                        <div className="bg-zinc-50 dark:bg-zinc-900 border p-6 rounded-3xl flex flex-col items-center">
                           <span className="text-[10px] font-black text-zinc-400 uppercase mb-1">إجمالي الإيرادات</span>
                           <span className="text-3xl font-mono font-black text-emerald-600">{fin.totalRevenues.toLocaleString()}</span>
                        </div>
                        <div className="bg-zinc-50 dark:bg-zinc-900 border p-6 rounded-3xl flex flex-col items-center">
                           <span className="text-[10px] font-black text-zinc-400 uppercase mb-1">إجمالي التكاليف والمصاريف</span>
                           <span className="text-3xl font-mono font-black text-rose-600">{(fin.cogs + fin.totalExpenses).toLocaleString()}</span>
                        </div>
                        <div className="bg-primary text-white p-6 rounded-3xl flex flex-col items-center shadow-xl">
                           <span className="text-[10px] font-black text-white/60 uppercase mb-1">صافي الربح النهائي</span>
                           <span className="text-3xl font-mono font-black">{fin.netProfit.toLocaleString()}</span>
                        </div>
                     </div>
                  </div>
                )}
                ...
                {reportType === 'TRADING' && (
                  <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 text-zinc-900 dark:text-zinc-100">
                     <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-emerald-50 dark:bg-emerald-950/20 p-6 rounded-3xl border border-emerald-100 flex flex-col items-center">
                           <span className="text-[10px] font-black text-emerald-600 uppercase mb-1">بضاعة أول المدة</span>
                           <span className="text-2xl font-mono font-black text-emerald-700">{fin.openingStock.toLocaleString()}</span>
                        </div>
                        <div className="bg-rose-50 dark:bg-rose-950/20 p-6 rounded-3xl border border-rose-100 flex flex-col items-center">
                           <span className="text-[10px] font-black text-rose-600 uppercase mb-1">صافي المشتريات (+)</span>
                           <span className="text-2xl font-mono font-black text-rose-700">{fin.purchasesTotal.toLocaleString()}</span>
                        </div>
                        <div className="bg-zinc-50 dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 flex flex-col items-center">
                           <span className="text-[10px] font-black text-zinc-400 uppercase mb-1">بضاعة آخر المدة (-)</span>
                           <span className="text-2xl font-mono font-black text-zinc-800 dark:text-zinc-200">{fin.closingStock.toLocaleString()}</span>
                        </div>
                        <div className="bg-primary text-white p-6 rounded-3xl flex flex-col items-center shadow-xl">
                           <span className="text-[10px] font-black text-white/60 uppercase mb-1">مجمل الربح (Gross Profit)</span>
                           <span className="text-2xl font-mono font-black">{fin.grossProfit.toLocaleString()}</span>
                        </div>
                     </div>
                     <div className="bg-zinc-50 dark:bg-zinc-900 p-6 rounded-[2rem] border border-zinc-200 dark:border-zinc-800">
                        <p className="text-xs font-bold leading-relaxed text-zinc-500">
                           المعادلة: بضاعة أول المدة ({fin.openingStock.toLocaleString()}) + المشتريات ({fin.purchasesTotal.toLocaleString()}) - بضاعة آخر المدة ({fin.closingStock.toLocaleString()}) = تكلفة البضاعة المباعة ({(fin.openingStock + fin.purchasesTotal - fin.closingStock).toLocaleString()}).
                        </p>
                     </div>
                  </div>
                )}
           ...
           {activeTab === 'OPENING_ENTRY' && (
             <div className="bg-white dark:bg-zinc-950 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl">
                <h3 className="text-2xl font-black mb-6 text-readable flex items-center gap-2"><Scale className="w-6 h-6 text-primary"/> سجل الأرصدة الافتتاحية للميزانية</h3>
                      ...
                      <tbody className="divide-y dark:divide-zinc-800 font-bold">
                         {openingEntries.map(e => (
                           <tr key={e.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                              ...
                              <td className="p-4 text-center no-print"><button onClick={() => handleDeleteEntry(e.id)} className="text-rose-500 p-2 hover:bg-rose-50 rounded-lg"><Trash2 className="w-4 h-4" /></button></td>
                           </tr>
                         ))}
                      </tbody>
                   </table>
           ...
           {activeTab === 'INVENTORY_TOOLS' && (
             <div className="bg-white dark:bg-zinc-950 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl">
                <h3 className="text-2xl font-black mb-6 text-readable flex items-center gap-2"><Box className="w-6 h-6 text-primary"/> سجلات الجرد الدوري الموثقة</h3>
                <div className="space-y-4">
                   {inventories.map(inv => (
                      <div key={inv.id} className="border dark:border-zinc-800 rounded-3xl overflow-hidden shadow-sm">
                         <div className={`p-4 flex justify-between items-center cursor-pointer transition-colors ${inv.type === 'OPENING' ? 'bg-emerald-500/10 hover:bg-emerald-500/20' : 'bg-rose-500/10 hover:bg-rose-500/20'}`} onClick={() => toggleInventory(inv.id)}>
                            <div className="flex items-center gap-4">
                               ...
                               <div>
                                  <span className={`px-4 py-1 rounded-full text-[9px] font-black border ${inv.type === 'OPENING' ? 'bg-emerald-500/20 text-emerald-700 border-emerald-500/30' : 'bg-rose-500/20 text-rose-700 border-rose-500/30'}`}>
                                     {inv.type === 'OPENING' ? 'جرد أول المدة' : 'جرد آخر المدة'}
                                  </span>
                            ...
                            <div className="flex items-center gap-6">
                               <div className="text-center">
                                  <span className="text-[9px] font-black text-zinc-400 uppercase block">إجمالي قيمة البضاعة</span>
                               ...
                               }} className="text-rose-500 p-2 hover:bg-rose-50 rounded-xl"><Trash2 className="w-4 h-4"/></button>
                            </div>
                         </div>
                         {(expandedInventories.has(inv.id) || printDetails) && (
                            <div className="p-4 bg-white dark:bg-zinc-900 border-t dark:border-zinc-800">
                               <table className="w-full text-right text-xs">
                                  <thead>
                                     <tr className="border-b dark:border-zinc-800 text-zinc-400 font-black h-10">
                                        <th className="pr-4">الصنف</th><th className="text-center">الكمية</th><th className="text-center">سعر التكلفة (Unit)</th><th className="text-center">الإجمالي</th>
                                     </tr>
                                  </thead>
                                  <tbody className="divide-y dark:divide-zinc-800 font-bold">
                                     {inv.items.map((item, idx) => (
                                        <tr key={idx} className="h-10 hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                               ...
                               {inv.notes && <div className="mt-4 p-3 bg-zinc-50 dark:bg-zinc-800 rounded-xl text-[11px] font-bold text-zinc-500 border border-dashed dark:border-zinc-700 italic">ملاحظات: {inv.notes}</div>}
                            </div>
                         )}
                      </div>
                   ))}
                   {inventories.length === 0 && <div className="p-20 text-center text-zinc-400 italic font-bold">لا يوجد سجلات جرد تفصيلية مسجلة حالياً</div>}
                </div>
             </div>
           )}
        </div>
      ...
      {isAddingEntry && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
           <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden p-8 animate-in zoom-in-95">
              <div className="flex items-center justify-between border-b dark:border-zinc-800 pb-5 mb-8">
                 <h3 className="text-2xl font-black text-readable">إضافة قيد ميزانية (افتتاحي)</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div className="flex flex-col gap-1.5 md:col-span-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mr-2">اسم الحساب</label>
                    <input list="opening-accounts-list" type="text" className="bg-zinc-50 dark:bg-zinc-950 border-2 border-zinc-100 dark:border-zinc-800 p-4 rounded-2xl font-black text-readable outline-none focus:border-primary transition-all" value={formData.accountName} onChange={e => setFormData({...formData, accountName: e.target.value})} placeholder="مثلاً: أثاث ومفروشات، رأس مال الشريك..." />
                    <datalist id="opening-accounts-list">
                       <optgroup label="الزبائن والموردين">{allParties.map(p => <option key={p.id} value={p.name} />)}</optgroup>
                       <optgroup label="الأقسام المحاسبية">{categories.map(c => <option key={c.id} value={c.name} />)}</optgroup>
                    </datalist>
                 </div>
                 ...
                 <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mr-2">مدين (Debitor)</label>
                 ...
                 <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mr-2">دائن (Creditor)</label>
              ...
              <div className="mt-8 flex justify-end gap-3 pt-6 border-t dark:border-zinc-800">
                 <button onClick={handleSaveEntry} className="bg-primary text-white px-12 py-4 rounded-2xl font-black shadow-xl hover:scale-105 transition-all text-lg">تثبيت القيد</button>
              </div>
           </div>
        </div>
      )}

      {/* Inventory Modal - REWORKED FOR DETAILED ENTRY */}
      {isAddingInventory && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
           <div className="bg-white dark:bg-zinc-900 w-full max-w-5xl rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-2xl p-8 animate-in zoom-in-95 flex flex-col max-h-[90vh]">
              <div className="flex items-center justify-between border-b dark:border-zinc-800 pb-5 mb-6 shrink-0">
                 <div className="flex items-center gap-3">
                    ...
                    <div>
                       <h3 className="text-xl font-black text-readable">تسجيل جرد {invType === 'OPENING' ? 'أول المدة' : 'آخر المدة'}</h3>
                       <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">تحديد كميات وتكاليف الأصناف المخزنية</p>
                    </div>
                 </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 shrink-0">
                 ...
                 <div className="flex flex-col gap-1.5 md:col-span-2">
                    <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mr-2">ملاحظات عامة عن الجرد</label>
              ...
              <div className="bg-zinc-50 dark:bg-zinc-950 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-inner mb-6 shrink-0">
                 <label className="text-[10px] font-black text-zinc-400 uppercase mb-3 block">إضافة أصناف للجرد</label>
                 <div className="relative">
                    <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 w-5 h-5" />
                    <input 
                       type="text" 
                       placeholder="ابحث عن مادة من المخزن لإضافتها..." 
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar border rounded-3xl mb-6 bg-white dark:bg-zinc-900">
                 <table className="w-full text-right border-collapse">
                    <thead className="bg-zinc-50 dark:bg-zinc-800 text-[10px] font-black uppercase sticky top-0 z-10 h-10">
                       <tr><th className="p-3 pr-6">اسم المادة</th><th className="p-3 text-center">الكمية الجردية</th><th className="p-3 text-center">سعر التكلفة (Unit)</th><th className="p-3 text-center">الإجمالي</th><th className="p-3"></th></tr>
                    </thead>
                    <tbody className="divide-y dark:divide-zinc-800 font-bold">
                       {invItems.map(it => (
                          <tr key={it.itemCode} className="h-12 hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                             ...
                             <td className="p-3 text-center">
                                <button onClick={() => setInvItems(invItems.filter(x => x.itemCode !== it.itemCode))} className="text-zinc-300 hover:text-rose-500"><X className="w-4 h-4" /></button>
                             </td>
                          </tr>
                       ))}
                       {invItems.length === 0 && (
                          <tr><td colSpan={5} className="p-20 text-center text-zinc-300 font-bold italic">لا يوجد أصناف في قائمة الجرد، قم بإضافة أصناف من البحث أعلاه</td></tr>
                       )}
                    </tbody>
                 </table>
              </div>
              ...
              <div className="shrink-0 flex items-center justify-between p-6 bg-zinc-900 rounded-[2rem] text-white shadow-xl">
                 <div className="flex gap-10">
                    <div className="text-center">
                       <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block">عدد الأصناف</span>
                       <span className="text-2xl font-mono font-black">{invItems.length}</span>
                    </div>
                    <div className="text-center">
                       <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest block">إجمالي قيمة القائمة الجردية</span>
                       <span className="text-3xl font-mono font-black text-emerald-400">{invItems.reduce((s,i) => s+i.total, 0).toLocaleString()}</span>
                    </div>
                 </div>
                 <button onClick={handleSaveInventoryRecord} className="bg-white text-zinc-900 px-12 py-4 rounded-2xl font-black text-lg hover:scale-105 transition-all">تثبيت واعتماد الجرد</button>
              </div>
           </div>
        </div>
      )}
