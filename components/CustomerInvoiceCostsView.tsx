...
const CustomerInvoiceCostsView: React.FC<CustomerInvoiceCostsViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500" dir="rtl">
      ...
      <div className="bg-white rounded-b-2xl border-2 border-zinc-300 overflow-hidden shadow-2xl">
        <div className="overflow-x-auto">
          <table className="w-full text-center border-collapse text-[12px]">
            <thead>
              <tr className="bg-rose-900 text-white font-black border-b-2 border-rose-950 h-12 uppercase tracking-tighter">
                <th className="p-2 border-l border-rose-800 w-24">تاريخ</th>
                <th className="p-2 border-l border-rose-800 w-20">اليوم</th>
                <th className="p-2 border-l border-rose-800 w-24">كود الصنف</th>
                <th className="p-2 border-l border-rose-800 text-right pr-6">اسم الصنف</th>
                <th className="p-2 border-l border-rose-800 w-16">الوحدة</th>
                <th className="p-2 border-l border-rose-800 w-24">السعر</th>
                <th className="p-2 border-l border-rose-800 w-20">الحركة</th>
                <th className="p-2 border-l border-rose-800 w-28 font-black text-sm">الكمية</th>
                <th className="p-2 border-l border-rose-800 text-right pr-4">البيان</th>
                ...
                <>
                  {filteredData.map((e, idx) => (
                    <tr key={e.id} className="h-12 bg-white hover:bg-rose-50 transition-colors">
                      ...
                      <td className="p-2 border-l border-zinc-200 font-mono font-black text-lg bg-zinc-50/50">{e.quantity.toLocaleString()}</td>
