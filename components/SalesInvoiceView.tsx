...
import { ArrowRight, Printer, Plus, Trash2, Save, X, Box, Clock, FileDown, User, Hash, HardDrive, ScrollText } from 'lucide-react';
...
const SalesInvoiceView: React.FC<SalesInvoiceViewProps> = ({ onBack }) => {
  ...
  const handleSaveInvoice = () => {
    ...
    // 3. Record Payment in Cash Journal (قبض نقدي)
    if (invoice.paidAmount! > 0) {
      ...
      const cashMove: CashEntry = {
        id: crypto.randomUUID(),
        date: invoice.date,
        statement: `مقبوضات مقابل فاتورة مبيعات رقم ${invNum} - العميل: ${invoice.customerName}`,
    ...
    setIsAdding(false);
    setNewInvoice({ invoiceNumber: '', customerName: '', date: new Date().toISOString().split('T')[0], items: [], usedMaterials: [], notes: '', paidAmount: 0 });
    alert('تم حفظ فاتورة المبيعات وتحديث الصندوق والمستودع بنجاح.');
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between no-print">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl font-black text-readable">المبيعات والفواتير</h2>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => exportToCSV(invoices, 'sales_invoices')} className="bg-zinc-800 text-white px-6 py-2.5 rounded-2xl font-black flex items-center gap-2">
        ...
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-2xl space-y-8 animate-in zoom-in-95 no-print">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="flex flex-col gap-1">
               ...
               <input type="text" placeholder="ترقيم تلقائي" className="bg-zinc-50 dark:bg-zinc-800 p-3 rounded-2xl border border-zinc-200 dark:border-zinc-700 outline-none font-bold" value={newInvoice.invoiceNumber} onChange={e => setNewInvoice({...newInvoice, invoiceNumber: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
               <label className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mr-1">المبلغ المدفوع (قبض مالي)</label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
             <div className="bg-zinc-50 dark:bg-zinc-800/50 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 space-y-4">
                <h4 className="text-sm font-black text-primary flex items-center gap-2 border-b border-zinc-200 dark:border-zinc-800 pb-2 uppercase tracking-widest">
                   <ScrollText className="w-4 h-4" /> بنود الفاتورة (يدوي)
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                   <input type="text" placeholder="اسم الصنف أو الوصف..." className="md:col-span-2 bg-white dark:bg-zinc-900 p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 font-bold outline-none" value={manualItem.name} onChange={e => setManualItem({...manualItem, name: e.target.value})} />
                   <div className="flex gap-2">
                ...
                <button onClick={handleAddManualItem} className="w-full bg-zinc-800 text-white p-3 rounded-xl shadow-lg hover:bg-zinc-700 transition-all font-bold flex items-center justify-center gap-2">
                   <Plus className="w-4 h-4" /> إضافة بند للفاتورة
                </button>
                         ...
                         {newInvoice.items?.map(item => (
                            <tr key={item.id}>
                               ...
                               <td className="p-2 text-center">
                                  <button onClick={() => setNewInvoice({...newInvoice, items: newInvoice.items?.filter(i => i.id !== item.id)})} className="text-rose-500 p-1 hover:bg-rose-50 dark:hover:bg-rose-900/10 rounded"><Trash2 className="w-4 h-4"/></button>
                               </td>
                            </tr>
                         ))}
                      </tbody>
             ...
             <div className="bg-rose-500/5 dark:bg-rose-500/10 p-6 rounded-3xl border border-rose-500/20 space-y-4">
                <h4 className="text-sm font-black text-rose-600 flex items-center gap-2 border-b border-rose-500/20 pb-2 uppercase tracking-widest">
                   <HardDrive className="w-4 h-4" /> المواد المستخدمة (خصم من المخزون)
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                   ...
                   <div className="flex gap-2">
                      ...
                      <button onClick={handleAddUsedMaterial} className="bg-rose-600 text-white p-3 rounded-xl shadow-lg hover:bg-rose-500 transition-all font-bold">خصم</button>
                   </div>
                </div>
                <div className="max-h-60 overflow-y-auto">
                   <table className="w-full text-xs">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center pt-6 border-t border-zinc-100 dark:border-zinc-800 gap-6">
             <div className="text-primary font-black text-xl bg-primary/5 px-8 py-3 rounded-2xl border border-primary/20 w-full md:w-auto text-center">
                {tafqeet(newInvoice.items?.reduce((s,i) => s + i.total, 0) || 0)}
             </div>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800 font-bold">
              {invoices.map(inv => (
                <tr key={inv.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors group">
                  <td className="p-4 font-black text-primary">#{inv.invoiceNumber}</td>
                  <td className="p-4 text-xs font-mono text-zinc-400">{inv.date}</td>
                  <td className="p-4">{inv.customerName}</td>
                  <td className="p-4 text-center font-black">{inv.totalAmount.toLocaleString()}</td>
                  <td className="p-4 text-center text-emerald-600">{inv.paidAmount?.toLocaleString() || '0'}</td>
                  <td className="p-4 text-center text-rose-500">{(inv.totalAmount - (inv.paidAmount || 0)).toLocaleString()}</td>
                  <td className="p-4 no-print text-center">
                    <button onClick={() => window.print()} className="p-2 text-zinc-400 hover:text-primary transition-all"><Printer className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
