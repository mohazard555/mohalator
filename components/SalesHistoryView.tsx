...
const SalesHistoryView: React.FC<SalesHistoryViewProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    if (savedInv) {
      ...
      // استخراج كافة المواد (أصناف مباعة + مواد مستخدمة) لغرض الفلترة
  ...
  const filteredInvoices = invoices.filter(inv => {
    // تعديل: البحث الآن يشمل رقم الفاتورة، اسم العميل، وأيضاً أسماء الأصناف داخل الفاتورة
    ...
    // فلترة بناءً على المواد المختارة من القائمة المنسدلة (إذا كانت القائمة غير فارغة)
    let matchItems = true;
    if (selectedItems.length > 0) {
      const invoiceAllItems = [
        ...inv.items.map(it => it.name),
        ...(inv.usedMaterials?.map(m => m.name) || [])
      ];
    ...
    <div className="space-y-6">
      ...
      <div className="print-only print-header flex justify-between items-center bg-rose-700 p-6 rounded-t-xl text-white mb-0 border-b-0">
        ...
        <div className="text-center">
          <h2 className="text-3xl font-black underline decoration-white/30 underline-offset-8">سجل المبيعات المفلتر</h2>
          <p className="text-xs mt-2 opacity-80 flex items-center justify-center gap-1">
            <Calendar className="w-3 h-3"/> {startDate || 'بداية السجل'} ← {endDate || 'اليوم'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="flex-1 min-w-[200px] flex flex-col gap-1">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl py-3 pr-12 outline-none font-bold focus:border-rose-700 transition-all text-readable"
                value={searchTerm}
          ...
          <div className="flex-1 min-w-[200px] flex flex-col gap-1 relative">
            ...
            <button 
              ...
              className="w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl py-3 px-4 flex items-center justify-between font-bold text-readable hover:bg-zinc-100 transition-all overflow-hidden"
            >
              <div className="flex items-center gap-2 truncate">
                <Package className="w-4 h-4 text-rose-500 shrink-0" />
                <span className="truncate">{selectedItems.length > 0 ? `مختار (${selectedItems.length}) أصناف` : 'جميع المواد'}</span>
              </div>
            ...
            {showItemDropdown && (
              <div className="absolute top-full right-0 left-0 mt-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl z-50 max-h-60 overflow-y-auto p-2 animate-in slide-in-from-top-2">
                <button 
                  ...
                  className="w-full text-right p-2 text-[10px] font-black text-rose-500 border-b mb-1 hover:bg-rose-50 rounded-lg"
                >
                  إعادة تعيين (الكل)
                </button>
                {uniqueItems.map(item => (
                  <div 
                    ...
                    className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${selectedItems.includes(item) ? 'bg-rose-500/10 text-rose-600' : 'hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                  >
      ...
      {selectedItems.length > 0 && (
        <div className="bg-emerald-500/5 dark:bg-emerald-950/20 p-6 rounded-3xl border border-emerald-500/20 shadow-inner animate-in zoom-in-95 print:mt-4 print:bg-transparent">
          <h4 className="text-sm font-black text-emerald-600 flex items-center gap-2 mb-4 uppercase tracking-widest border-b border-emerald-500/10 pb-2">
             <Filter className="w-4 h-4" /> تحليل المواد المختارة للفترة المحددة
          </h4>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
             {materialsAnalysis.map((mat, i) => (
               <div key={i} className="bg-white dark:bg-zinc-900 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-900/50 shadow-sm flex flex-col items-center text-center">
                  <span className="text-[10px] font-black text-zinc-400 mb-1 truncate w-full">{mat.name}</span>
                  <span className="text-xl font-black text-emerald-600 font-mono">{mat.qty}</span>
                  <span className="text-[8px] font-bold text-zinc-500">وحدة / قطعة</span>
                  {mat.val > 0 && <span className="text-[9px] font-black text-amber-600 mt-1">{mat.val.toLocaleString()} ل.س</span>}
               </div>
             ))}
          </div>
        </div>
              ...
              {filteredInvoices.length === 0 ? (
                ...
                filteredInvoices.map((inv, idx) => (
                  <tr key={inv.id} className="hover:bg-rose-50 dark:hover:bg-zinc-800/30 transition-colors h-12">
                    ...
                    <td className="p-2 border-l border-zinc-100 dark:border-zinc-800 text-center font-black text-rose-600 font-mono text-xs bg-rose-50/30 dark:bg-rose-950/20 print:bg-transparent">
